...
from ._utils import InstanceStandardize
...
class GridwatchDataset(torch.utils.data.Dataset):
    ...
    def _get_gridwatch_dataset(self, train_test_ratio):
        ...
        y_full = np.zeros(
            (self.num_samples, self.num_points, 1), dtype=np.float32)
        for i in range(self.num_samples):
            y_full[i] = demand_values[i *
                                      self.num_points:(i+1)*self.num_points].values.reshape(-1, 1)

        y_full = torch.tensor(y_full)

        num_train = int(self.num_samples * train_test_ratio)
        train_indices = np.random.choice(
            self.num_samples, num_train, replace=False)
